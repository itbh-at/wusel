...
Nextcloud's official client keeps one Qt/C++ code base for Windows/macOS/Linux,
so its Linux VFS arrives as an attachment: an external FUSE process
(https://github.com/opencloud-eu/openvfs[openvfs]) dehydrates a directory the
mirroring engine has already synced, and is driven over a socket
(xref:explanation/compared-to-the-official-client.adoc[Compared to the official Nextcloud client]). `wusel` turns this around: a platform-independent engine plus
interchangeable, thin frontends, with the virtual filesystem as the starting
point instead of a layer on top.
...
wusel-fileprovider  (macOS)   — File Provider ext.   → intents   [experimental]
