...
Here is what that looks like — a real session against a real Nextcloud, which is
all a server ever gives you: no desktop, just the mount and the terminal.

// Recorded by scripts/demo-nextcloud.sh; the player is lazy-loaded by the UI bundle.
[subs=attributes+]
++++
<div style="clear: both; margin: 1.5rem 0; border: 1px solid #d7dbe0; border-radius: 8px; overflow: hidden;">
  <div class="asciinema" data-cast="{attachmentsdir}/wusel-demo.cast" data-cols="90" data-rows="28"></div>
</div>
++++
...
emblem from — the daemon serves this socket alongside the mount:
