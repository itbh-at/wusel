...
sections below say which parts. It belongs next to xref:architecture.adoc[Architecture],
which names the three frontends, and xref:concurrency.adoc[Concurrency], which
designs the state machine they would share.
...
Today the scripts in xref:fsm-scripts.adoc[The operation scripts] *are* the FUSE
...
In the spirit of the staged gates in xref:concurrency.adoc[Concurrency], the
...
* **A timeline.** This sits behind everything in xref:roadmap.adoc[Roadmap].
