...
= Known limitations
:description: Behaviour that is understood, reproducible and not yet fixed — what you see, why it is so, and what removing it would take.
...
xref:roadmap.adoc[Roadmap] says what is *planned*, and
xref:troubleshooting.adoc[Troubleshooting] answers "my mount is broken, what
...
exactly this (xref:concurrency.adoc[Concurrency]). The command refuses because
...
see xref:file-manager.adoc[File-manager integration]. Previews are not covered
...
xref:file-manager.adoc#xattr[`user.wusel.state`], a deliberate contract that any
